...
// Uses the nixos/nix base so nix provides go/cosign/syft/gcc reproducibly.
func (m *Opslifyd) Integration(ctx context.Context, source *dagger.Directory) (string, error) {
	return dag.Container().
		From("nixos/nix:latest").
		WithMountedCache("/nix/store", dag.CacheVolume("opslifyd-nixstore")).
		...
		WithExec([]string{"sh", "-c",
			"cosign generate-key-pair && " +
				"export OPSLIFY_COSIGN_KEY=$PWD/cosign.key OPSLIFY_COSIGN_PUBKEY=$PWD/cosign.pub && " +
				"go test -v -run Integration ./internal/env/..."}).
		Stdout(ctx)
}
