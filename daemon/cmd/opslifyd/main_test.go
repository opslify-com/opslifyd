...
func TestDaemonOptionsWireSecretsService(t *testing.T) {
	...
	opts := daemonOptions(cfg, "/run/opslify/api.sock", "opslify", nil, nil, nil, nil, svc, nil, nil, nil, nil, nil, discardLog())
...
func TestDaemonOptionsCarryEveryStatefulDependency(t *testing.T) {
	...
	opts := daemonOptions(cfg, "/run/opslify/api.sock", "opslify", verifier, mgr, projects, vault, svc, nil, nil, nil, nil, nil, discardLog())
